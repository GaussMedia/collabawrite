...
<script type="text/javascript" >
 $(document).ready(function() { 
     //$('.file').hide();
       ...
       var coverimage = $('#half_cover').attr('style');
     //alert(coverimage);
     $('#hidprofilecover').val(coverimage);
     var userprofileimage = $('.myprofile').attr('src');
     //alert(userprofileimage);
     $('#hidprofileimage').val(userprofileimage);
            $('#fileUpload').show();
             $('span.button').show();
            $(".logohide").hide();
            $("#imageform").show();
           ...
           // $(".userProfileDesc").find('textarea').val($prD);
            ...
            $('.profile').html('<div class="file"><input type="file" id="profilepic" onchange="displayPreview1(this.files);" name="profilepic" /><span id="preview1" class="button"></span></div>');
            $(this).hide();
           ...
           dataType : 'json',
                success : function(res){
                    //alert(res+res.profile_cover+res.image);
                    var newcover = "background:url(webadmin/upload/usercover/original/"+res.profile_cover+")";
                    var newimage = "webadmin/upload/usercover/original/"+res.image;
                    //alert(newcover);
                    //alert(newimage);
                    $('.profile').html('<img src="'+newimage+'" class="media-object myprofile ">');
                    $('#half_cover').attr('style',newcover);
                    $('#imageform').show();
                    $(".logohide").show();
      ...
      function onFileLoad1(e) { 
		...
		//$("#preview").append('<img src="ajaximage/loader.gif" alt="Uploading...."/>');
                //$('#preview').show();e.target.result
                $('#preview1').css('background', 'url(' + e.target.result + ')');
...
//                    }).submit();
		...
		//$("#preview").append('<img src="ajaximage/loader.gif" alt="Uploading...."/>');
                //$('#preview').show();e.target.result
                $('#half_cover').css('background', 'url(' + e.target.result + ')');
...
<div class="row-fluid">
    ...
    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
     ...
     ?>
      ...
      ?>
            <img class="" src="webadmin/upload/userprofile/original/<?php echo $sessionuser['image'];  ?>" alt="">
            <?php
 }
 }
              }
...
<!--               <a class="btn btn-success" id="edituser" href="javascript:void(0)"><i class="icon-white icon-edit"></i> Edit Profile</a>-->
    ...
    <div class="wrapper">
       ...
       <?php
            $sql_ep = "SELECT * FROM drafts WHERE Status ='1' AND author='$fetch_profile[id]' ORDER by id DESC ";
...
?>         <div class="span4 botm_margin20">
           <div class="location_img">
               <a href="collection?collection_name=<?=$fetchcoll['id']?>">
                   <img src="webadmin/upload/collection/thumb/<?=$fetchcoll['image']?>" alt=""/><div class="location_text width89">
    ...
    $fetch_post=$obj->fetch_one('drafts',"`id`='".$fetch_rqm['recommend_post']."'");
      ...
      if($i%4 == 0)
      {
          echo '</div><div class="row-fluid">';
                 ...
                 ?>
            <div class="span4">
            <p><a href="post_more?post=<?php echo base64_encode($fetch_post['id']);?>"><strong>
                <?php 
                if($fetch_post['title'] == ''){
                    echo 'Untitled';
                }
...
<body>
    <input type="hidden" name="hidprofilecover" id="hidprofilecover" value="">
    <input type="hidden" name="hidprofileimager" id="hidprofileimage" value="">
<div class="row-fluid">
...
<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
    ...
    <div class="pepl text-center">
   <form  id="imageform" action="cover_upload" method="post" enctype="multipart/form-data">
      <div class="cover_table">
      
   	<div class="cover_row">
            <?php
            
            if($sessionuser['profile_cover'] == "")
            {
               ?>
                   <div class="half_cover" style="background:url(http://reportedly.pnf-sites.info/no-img.jpg);">
                   <?php 
            }else{
                   
             ?>
            <div class="half_cover imageFormDiv" id="half_cover" style="background:url(webadmin/upload/usercover/original/<?php echo $sessionuser['profile_cover']?>);">
                <?php
           }
            ?>
          <?php
...
<!--        </form>-->
        ...
        <div class="cover_row">
            ...
            {
                ?>
            <img title="<?php echo $sessionuser['fullname'];?>" class="media-object myprofile " src="<?php echo $sessionuser['image'];?>" alt=""/>
            <?php
            }
            else
            {
                ...
                {
            ?>
            <img title="<?=$sessionuser['fullname']?>" class="media-object myprofile " src="https://graph.facebook.com/<?=$sessionuser['username']?>/picture?width=200&height=200">
            <?php
                }
 else {
     if($sessionuser['image'] == ""){
     ?>
         ...
         ?>
          <img class="media-object myprofile" src="webadmin/upload/userprofile/original/<?php echo $sessionuser['image'];?>" alt="">
          <?php
     }
 }
              }
    ...
    <div class="wrapper">
   	...
   	<div class="wrapper_inner">
       <?php
            $sql_ep = "SELECT * FROM drafts WHERE status ='1' AND author='$sessionuser[id]' ORDER by id DESC ";
            $res_ep = mysql_query($sql_ep)or die(mysql_error());
            if(mysql_num_rows($res_ep)>0){
            ?>
       <div class="row-fluid">
       <hr/>
       <span class="zero_margin"><a href="#" class="light_grey">Latest Reports</a></span>
...
$res_query_collections = mysql_query($query_collections)or die(mysql_error());
 if(mysql_num_rows($res_query_collections)>0){
    ?>
       <div class="row-fluid">
    <hr/>
...
?>         <div class="span4 botm_margin20">
           <div class="location_img">
               <a href="collection?collection_name=<?=$fetchcoll['id']?>">
                   ...
                   ?>
                   <img src="timthumb.php?src=webadmin/upload/collection/original/<?=$fetchcoll['image']?>&h=<?php echo $height; ?>&w=<?php echo $width; ?>&zc=1" alt=""/><div class="location_text width89">
   <?php echo $fetchcoll['collection_name']; ?> <br/>
                                   <?php
$sqk="SELECT * FROM drafts WHERE collection_id='".$fetchcoll['id']."' AND status='1'";
$fetch_post=mysql_query($sqk)or die (mysql_error());
...
?>
       ...
       $sql_rqm = " SELECT *
                    ...
                    LIMIT 6  ";
      $res_rqm = mysql_query($sql_rqm)or die(mysql_error());
      //$arr_recm = mysql_fetch_array($res_rqm);
    ...
    $fetch_post=$obj->fetch_one('drafts',"`id`='".$fetch_rqm['recommend_post']."'");
      ...
      if($i%4 == 0)
      {
          ...
          ?>
             ...
             <div class="span4">
            <p><a href="post_more?post=<?php echo base64_encode($fetch_post['id']);?>"><strong><?php
            if($fetch_post['title']  ==  ""){
                echo 'Untitled';
            }else{
            echo substr($fetch_post['title'],'0','31');
            }?></strong></a></p>
			<p><a href="collection.php?collection_name=<?=$fetch_collec['id'];?>" class="light_grey">in <?php echo $fetch_collec['collection_name']; ?></a></p>
              </div>
            <?php

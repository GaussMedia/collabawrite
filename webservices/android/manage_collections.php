      ...
      <table class="table table-bordered table-striped data-table" id="tbl1">
        ...
        <tr class="gradeA">
          <td class="tc"><input type="checkbox" class="checkbox"  name='chkall[]' value="<?php echo $fetch[$i]['id'];?>" />
            <?php
                  //echo $fetch['id'];?></td>
          <td><?php echo ucwords($fetch[$i]['collection_name']); ?></td>
          <td>
		  <?php echo substr($fetch[$i]['collection'],'0','50')."..."; ?>
          <a href="#">read more</a>
          </td>
           <td><?php $author_id=$fetch[$i]['collection_author'];
           $fetch_auther=$obj->fetch_one('twitter_users',"`id`='".$author_id."'");
           echo $fetch_auther['fullname'];
           ?></td>
           <td>
               <img width='100px' height='120px' src='../ajaximage/uploads/<?php echo $fetch[$i]['image'];?>'
           </td>
           <td><?php echo $fetch[$i]['contribute_type'];?></td>
          <td class="center"><?php
...
<?php
          ...
          <a href="index.php?mode=edit_collection&collection_id=<?php echo base64_encode($fetch[$i]['id']); ?>">

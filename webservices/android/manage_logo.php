...
<input type="hidden" id="dbtable" value="logo" />
  ...
  <!--<span>
    ...
    <a href="index.php?mode=addlogo">
    <button type="button" class="btn btn-error">Add Logo</button>
    </a>
    </span>
...
<!--		   <th>Actions</th>-->
        ...
        <tr class="gradeA">
         <!-- <td class="tc"><input type="checkbox" class="checkbox"  name='chkall[]' value="<?php echo $fetch[$i]['id'];?>" />-->
            <?php
                  //echo $fetch['id'];?></td>
          <td><?php echo ucwords($fetch[$i]['logo_title']); ?></td>
           <td><img src="upload/logo/real/<?php echo $fetch[$i]['logo_image']; ?>" /></td>
          <td class="center"><?php
...
<!--          <td>
          ...
          <span>
          <a href="index.php?mode=edit_blog&blog_id=<?php echo base64_encode($fetch[$i]['id']); ?>">

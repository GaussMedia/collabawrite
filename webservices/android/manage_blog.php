...
<input type="hidden" id="dbtable" value="blog" />
      ...
      <table class="table table-bordered table-striped data-table" id="tbl1">
        ...
        <tr class="gradeA">
          <td class="tc"><input type="checkbox" class="checkbox"  name='chkall[]' value="<?php echo $fetch[$i]['id'];?>" />
            <?php
                  //echo $fetch['id'];?></td>
          <td><?php echo ucwords($fetch[$i]['blog_title']); ?></td>
          <td>
		  <?php echo substr($fetch[$i]['blog_des'],'0','50')."..."; ?>
          <a href="#">read more</a>
          </td>
           <td><img src="upload/blog/thumb/<?php echo $fetch[$i]['blog_img']; ?>" /></td>
          <td class="center"><?php

...
<input type="hidden" id="dbtable" value="top_navs" />
  <div class="widget-header"> <span class="icon-list"></span>
    ...
    <span>
    <a href="index.php?mode=add_top_navs">
    <button type="button" class="btn btn-error">Add Top Navs</button>
    </a>
    </span>
  </div>
  <!-- .widget-header -->
      ...
      <table class="table table-bordered table-striped data-table" id="tbl1">
        ...
        <tr class="gradeA">
          <td class="tc"><input type="checkbox" class="checkbox"  name='chkall[]' value="<?php echo $fetch[$i]['id'];?>" />
            <?php
                  //echo $fetch['id'];?></td>
          <td><?php echo ucwords($fetch[$i]['nav_title']); ?></td>
		   <td><?php echo ucwords($fetch[$i]['page_title']); ?></td>
          <td class="center"><?php
...
<?php
          ...
          <!-- Edit Button-->
          <span>
          <a href="index.php?mode=edit_top_navs&top_nav_id=<?php echo base64_encode($fetch[$i]['id']); ?>">

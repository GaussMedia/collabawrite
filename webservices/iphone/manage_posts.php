...
<input type="hidden" id="dbtable" value="posts" />
      ...
      <table class="table table-bordered table-striped data-table" id="tbl1">
        ...
        <tr class="gradeA">
          <td class="tc"><input type="checkbox" class="checkbox"  name='chkall[]' value="<?php echo $fetch[$i]['id'];?>" />
            <?php
                  //echo $fetch['id'];?></td>
          <td><?php echo ucwords($fetch[$i]['title']); ?></td>
          <td>
		  <?php echo substr($fetch[$i]['post'],'0','50')."..."; ?>
          <a href="#">read more</a>
          </td>
          <td>
              <img src="../ajaximage/uploads/<?php echo $fetch[$i]['image'];?>" width="120px" height="100px">
          </td>
           ...
           <td><?php $col_id=$fetch[$i]['collection_id'];
...
<?php
          ...
          <!-- Edit Button-->
          <span>
          <a href="# <?php ///<!-- index.php?mode=edit_blog&blog_id= -->echo base64_encode($fetch[$i]['id']); ?>">

...
<input type="hidden" id="dbtable" value="employees" />
<input type="hidden" id="paytable" value="payments" />
  <div class="widget-header"> <span class="icon-list"></span>
    ...
    <span>
    <a href="index.php?mode=add_employee">
    <button type="button" class="btn btn-error">Add Employee</button>
    </a>
    </span>
 ...
 ?>
      ...
      <table class="table table-bordered table-striped data-table">
        ...
        <?php
                ...
                ?>
        <tr class="gradeA">
          <td class="tc"><input type="checkbox" class="checkbox"  name='chkall[]' value="<?php echo $fetch[$i]['Employee_id'];?>" />
            <?php
                  //echo $fetch['id'];?></td>
          <td><?php echo ucwords($fetch[$i]['first_name']); ?></td>
		   <td><?php echo ucwords($fetch[$i]['last_name']); ?></td>
		   <td><?php echo ucwords($fetch[$i]['email_address']); ?></td>
		   <td><?php echo ucwords($fetch[$i]['company_name']); ?> 
<!-- <a href="index.php?mode=company_details&company_id=<?php //echo base64_encode($fetch[$i]['company_id']);?>" class=""  id="" >   </a><br><a href="#" class="big-link pop"  id="<?php //echo $fetch[$i]['company_id'];?>" >
...
<br />
		  ...
		  $status=$fetch[$i]['status'];
		   if($fetch[$i]['status']=='1')
                {
		$id=$fetch[$i]['Employee_id'];
					?>
               
<input type="button"  class="btn publish btn-success" id="<?php  echo $fetch[$i]['Employee_id'];?>" value="Publish" />
<?php
}
else
{
?>
 <input  type="button" class="btn publish btn-error" id="<?php echo $fetch[$i]['Employee_id'];?>" value="Unpublish" />
<?php
				   }
				    ?></td>
          <td>
          <span><input  type="button" id="<?php  echo $fetch[$i]['Employee_id'];?>" class="btn delete btn-error" value="Delete" />
          </span>
<!--              <span>
              <a href="index.php?mode=reply_pwd&company_id=<?php echo base64_encode($fetch[$i]['Employee_id']); ?>">
          <img src="images/mail_reply.png"></a>
          </span>-->
          <span>
          <a href="index.php?mode=edit_employee&Employee_id=<?php echo base64_encode($fetch[$i]['Employee_id']); ?>">
